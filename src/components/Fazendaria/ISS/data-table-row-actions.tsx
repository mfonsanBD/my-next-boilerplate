...
type SelectedData = z.infer<typeof iss>
...
export function DataTableRowActions<TData>({
  ...
  const empresa = iss.parse(row.original)

  const handleEditDialog = (empresa: SelectedData) => {
    setSelectedToEdit(empresa)
    setOpenEdit(true)
  }

  const handleDeleteDialog = (empresa: SelectedData) => {
    setSelectedToDelete(empresa)
    setOpenDelete(true)
  }
  ...
  const onSubmit = useCallback(
    async (data: any) => {
      let newData: any = { ...data, id: empresa.id }
      ...
      await axios
        .patch('/api/fazendaria/iss', newData)
        .then((response) => {
          setOpenEdit(false)
          toast.success(response.data.message)
          mutate('/api/fazendaria/iss')
        })
        .catch((error) => {
  ...
  const handleDelete = useCallback(async (data: any) => {
    ...
    await axios
      .delete('/api/fazendaria/iss', { data: { id: data } })
      .then((response) => {
        setOpenDelete(false)
        toast.success(response.data.message)
        mutate('/api/fazendaria/iss')
  ...
  const [phoneType, setPhoneType] = useState(
    empresa.telefone.length === 14 ? 'fixo' : 'celular',
  )
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          <DropdownMenuItem
            className="cursor-pointer"
            onClick={() => handleEditDialog(empresa)}
          >
            Editar
          </DropdownMenuItem>
          ...
          <DropdownMenuItem
            className="cursor-pointer"
            onClick={() => handleDeleteDialog(empresa)}
          >
            Excluir
          </DropdownMenuItem>
        </DropdownMenuContent>
      ...
      >
        <AlertDialogContent className="max-w-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle>Alterar Empresa</AlertDialogTitle>
          </AlertDialogHeader>
