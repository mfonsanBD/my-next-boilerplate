...
export function DataTableRowActions<TData>({
  ...
  const { data } = useSWR('/api/activitytype', fetcher)
  const types = SelectMapper(data?.activityTypes)
  ...
  const onSubmit = useCallback(
    async (data: any) => {
      ...
      await axios
        .patch('/api/postura/ambulantes', rest)
        .then((response) => {
          setOpenEdit(false)
    ...
    <>
      ...
      >
        <AlertDialogContent className="max-w-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle>Alterar Ambulante</AlertDialogTitle>
          </AlertDialogHeader>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 items-end gap-4">
                  ...
                  <div className="col-span-full">
                    <Controller
                      name="activityType"
                      control={control}
                      defaultValue={selectedToEdit?.atividadeId || ''}
